...
Some locomotion features exhibit a correlation with the frog's body-size. For instance, the speed of a moving animal is usually increased with body-size. To make values comparable across genotypic groups, body-size normalization is implemented, by measuring the average Euclidean distance between the `Heart_Center` and the `Tail_1` body-part. For Juvenile frogs the distance of `Heart_Center` and `Tail_Stem` is used. We use this `body_size_mean` feature to normalize speed, acceleration and distance based features. To map the body-size normalized values back into physiological ranges, one can multiply the within group `body_size_mean` averages.
...
## Computed features and statistics
